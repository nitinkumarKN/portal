...
import { 
  Send, Save, Briefcase, MapPin, IndianRupee, Calendar, 
  Award, Users, FileText, ChevronRight, ChevronLeft, X 
} from 'lucide-react';
...
export default function PostJob() {
  ...
  const checkCompanyStatus = async () => {
    try {
      ...
      setCompanyApproved(data.approved);
      if (!data.approved) {
  ...
  const handleSubmit = async (submitType) => {
    ...
    if (!companyApproved) {
      toast.error('Your company is not approved yet. Please wait for admin approval.');
      return;
    }
    ...
    try {
      const submissionData = {
        ...formData,
        status: submitType === 'draft' ? 'Draft' : 'Pending Approval'
      };

      await axios.post('/api/company/job', submissionData);
      ...
      console.error('Job posting error:', error.response);
      const errorMessage = error.response?.data?.message || 'Failed to post job';
      toast.error(errorMessage);
      
      // Show specific error for unapproved company
      if (error.response?.status === 403) {
        toast.error('Please contact admin to approve your company account');
      }
    } finally {
      setLoading(false);
    }
  ...
  // Show warning if company not approved
      ...
      <div className="max-w-4xl mx-auto">
        <div className="card bg-orange-50 border-orange-200">
          <div className="text-center py-12">
            ...
            <h2 className="text-2xl font-bold text-gray-900 mb-2">Company Not Approved</h2>
            <p className="text-gray-600 mb-6">
              Your company account is pending admin approval. You cannot post jobs until your account is verified.
            </p>
            <button
        ...
        <div className="flex items-start space-x-3">
          ...
          <div>
            ...
            <ul className="text-sm text-blue-700 space-y-1 list-disc list-inside">
              ...
              <li>Jobs require admin approval before becoming visible to students</li>
              <li>Duplicate job titles are not allowed for the same company</li>
